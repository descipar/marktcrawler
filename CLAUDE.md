...
`app/version.py` liefert die aktuelle Commit-Info (`get_current_version()`) via `subprocess` (Dev) oder Env-Vars `GIT_COMMIT/GIT_DATE/GIT_MESSAGE` (Docker Build-Args). `get_available_updates(hash)` ruft die GitHub-API (`/repos/{owner}/{repo}/compare/{hash}...main`) ab und zeigt neuere Commits mit Message + Datum. Ergebnis wird 1h im Memory gecacht. Repo-Erkennung via `GITHUB_REPO`-Env-Var (in `docker-compose.yml` vorbelegt) oder automatisch aus `git remote get-url origin`. Fällt bei Offline/private Repo stumm aus (`None`).
