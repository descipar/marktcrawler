...
`app/version.py` liefert die aktuelle Commit-Info (`get_current_version()`): Priorität ist `app/_version.py` (beim `docker build` via `scripts/bake_version.py` eingebrannt) → Env-Vars `GIT_COMMIT/GIT_DATE/GIT_MESSAGE` → `git`-Subprocess (Dev). `scripts/bake_version.py` liest `.git/logs/refs/heads/<branch>` direkt (kein `git`-Binary nötig im Docker-Build-Kontext) und filtert "Fast-forward"-/Checkout-Einträge heraus (`_last_commit_from_log()` iteriert rückwärts und überspringt alle Zeilen, die nicht mit `"commit"` beginnen). `get_available_updates(hash)` ruft die GitHub-API (`/repos/{owner}/{repo}/compare/{hash}...main`) on-demand über `GET /api/check-updates` ab — kein Auto-Load, kein Cache. Commit-Hashes auf der Info-Seite sind direkte GitHub-Links. Repo-Erkennung via `GITHUB_REPO`-Env-Var (in `docker-compose.yml` vorbelegt) oder automatisch aus `git remote get-url origin`. Fällt bei Offline/private Repo stumm aus (`None`).
